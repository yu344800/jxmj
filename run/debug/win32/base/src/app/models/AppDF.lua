...
if device.platform == "ios" then
	appdf.CLIENT_UPDATE_URL 						= "https://itunes.apple.com/cn/app/%E6%BD%AE%E7%89%9B%E6%A3%8B%E7%89%8C/id1352860019?mt=8"
elseif device.platform == "android" then
	appdf.CLIENT_UPDATE_URL 						= "https://fir.im/cnqp"					
end

appdf.PROXY_ID 								= 'authorize' 							-- 代理标识
appdf.LINK_URL								= "https://aq3jnf.mlinks.cc/AbOb"		-- 魔窗地址

-- cmd版本
appdf.CMD_VERSION = 1 --@cmd_version
--base资源版本
		...
		if node then
			...
			elseif string.find(type, "Sprite") ~= nil then
				if node:isTouchEnabled() == false then
					...
					node:addNodeEventListener(cc.NODE_TOUCH_EVENT, function(e)
				...
				-- checkBox textfield
				node:addEventListener(function(v, e)
					if callback ~= nil then
...
function appdf.registerNodeEvent(parentNode)
	parentNode:registerScriptHandler(function(eventType)
