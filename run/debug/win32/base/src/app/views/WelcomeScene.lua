...
-- local URL_REQUEST = "http://192.168.0.80:8088" 			--@jam
local URL_REQUEST = "http://192.168.2.103:8066" 			--@xu

-- local URL_REQUEST = "http://120.78.147.74:8080" 	--@crqp-we
...
function WelcomeScene:onCreate()
    ...
    self._totalTips = cc.Label:createWithTTF("", "fonts/round_body.ttf", 24)
        :setTextColor(cc.c4b(234, 255, 255, 255))
        :setName("text_tip")
	...
	-- 闪屏动画
	self.mBackGround:runAction(cc.Sequence:create(
		...
		local msg = "网络连接异常..."
		 if type(datatable) == "table" then	 		
            ...
            if databuffer then
				...
				-- 微信登录开关
				local wxLogon = databuffer.wxLogon ~= nil and databuffer.wxLogon == 0 and true or false
				
				cc.UserDefault:getInstance():setBoolForKey("wxLogon",wxLogon)
			...
			-- 提审模式
			appdf.onHttpJsionTable(URL_REQUEST .. "/WS/MobileInterface.ashx","GET","action=iosnotappstorepayswitch",function(datatable)
				msg = "网络连接异常..."
				...
				if type(datatable) == "table" then
					...
					if databuffer then
						msg = nil
						if device.platform == "windows" then
							cc.UserDefault:getInstance():setBoolForKey("wxLogon", true)
							...
							this:EnterClient()
						elseif databuffer.State == 1 and ClientConfig.UPDATE_TO_APPSTORE == true then
							cc.UserDefault:getInstance():setBoolForKey("wxLogon", false)
							this:EnterClient()
						else
							self:excuteExtraCmd(self._newCmdVersion)
