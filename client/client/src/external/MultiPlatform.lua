...
function MultiPlatform:getMachineId()
	...
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getMachineId then
		...
		local id = cc.UserDefault:getInstance():getStringForKey("MachineID")
		if not id or id == "" then
			id = tostring(os.time())
			cc.UserDefault:getInstance():setStringForKey("MachineID", id)
		end
		return id
	end	
end
...
-- 分享到指定平台
function MultiPlatform:shareToTarget( target, callback, title, content, url, img, imgOnly, shareKey, shareValue )
	if nil == callback or type(callback) ~= "function" then
		return false, "need callback function"
	end
	-- 判断微信配置
	...
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).shareToTarget then
		return g_var(PLATFORM[plat]).shareToTarget( target, title, content, url, img, imgOnly, callback, shareKey, shareValue )
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return false, msg		
...
-- 获取魔窗id
function MultiPlatform:getMagicWindowValue()
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getMagicWindowValue then
		return g_var(PLATFORM[plat]).getMagicWindowValue( )
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return false, msg		
...
-- 初始定位
function MultiPlatform:initLocationManager()
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).initLocationManager then
		return g_var(PLATFORM[plat]).initLocationManager()
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return false, msg		
...
-- 获取定位信息
function MultiPlatform:getUserLocation()
	local plat = self:getSupportPlatform()
	if nil ~= g_var(PLATFORM[plat]) and nil ~= g_var(PLATFORM[plat]).getUserLocation then
		return g_var(PLATFORM[plat]).getUserLocation()
	else
		local msg = "unknow platform ==> " .. plat
		print(msg)
		return false, msg		
