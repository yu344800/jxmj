...
function ExternalFun.read_netdata(keyTable, dataBuffer)
    ...
    for k, v in pairs(keyTable) do
        ...
        elseif 'score' == keyType then
            keyFun = function()
                return dataBuffer:readscore(int64):getvalue()
            end
        ...
        elseif 'table' == keyType then
            cmd_table[key] =
                ExternalFun.readTableHelper({dTable = keys['d'], lentable = lenT, buffer = dataBuffer, strkey = key})
...
function ExternalFun.loadRootCSB(csbFile, parent)
    local rootlayer = ccui.Layout:create():setContentSize(1335, 750):setScale(yl.WIDTH / 1335) --这个是资源设计尺寸
    if nil ~= parent then
...
function ExternalFun.registerNodeEvent(node)
    ...
    local function onNodeEvent(event)
        if event == 'enter' and nil ~= node.onEnter then
            node:onEnter()
        elseif event == 'enterTransitionFinish' and nil ~= node.onEnterTransitionFinish then
...
function ExternalFun.playGameBackgroudAudio()
    if GlobalUserItem.bVoiceAble then
        AudioEngine.playMusic('sound/game_backgroud.wav', true)
    end
end
...
--叫友快捷文本语音
function ExternalFun.playGameSoundEffect(path)
    if GlobalUserItem.bSoundAble then
        if GlobalUserItem.GameSoundLanguage == 'pt' then
            path =
                string.format(
                'sound/quick_chat/%s_jy_%s/message%s.mp3',
                GlobalUserItem.GameSoundLanguage,
                GlobalUserItem.cbGender == 0 and 'man' or 'woman',
                path
            )
        elseif GlobalUserItem.GameSoundLanguage == 'cs' then
            path =
                string.format(
                'sound/quick_chat/%s_jy_%s/message%s.mp3',
                GlobalUserItem.GameSoundLanguage,
...
--这是麻将出牌语音
function ExternalFun.playGameCardSound(path)
    if GlobalUserItem.bSoundAble then
        if GlobalUserItem.GameSoundLanguage == 'pt' then
            path =
                string.format(
...
--这是叫友棋牌出牌语音
function ExternalFun.GameCardSound(path)
    if GlobalUserItem.bSoundAble then
        if GlobalUserItem.GameSoundLanguage == 'pt' then
            path =
                string.format(
                'sound/jy_pt_card_%s/card_%s.mp3',
                GlobalUserItem.cbGender == 0 and 'man' or 'woman',
                path
            )
            AudioEngine.playEffect(path, false)
        elseif GlobalUserItem.GameSoundLanguage == 'cs' then
            path =
                string.format(
                'sound/jy_cs_card_%s/card_%s.mp3',
                GlobalUserItem.cbGender == 0 and 'man' or 'woman',
                path
            )
            AudioEngine.playEffect(path, false)
...
--获取缩略字符串
function ExternalFun.GetShortString(sName, nMaxCount, nShowCount)
...
local TouchFilter =
    class(
    'TouchFilter',
    function(showTime, autohide, msg)
