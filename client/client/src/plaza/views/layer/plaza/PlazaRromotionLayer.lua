...
PlazaRromotionLayer.WIDGET_TAG = {}
local WIDGET_TAG = PlazaRromotionLayer.WIDGET_TAG

function PlazaRromotionLayer:ctor(scene)
    appdf.registerTouchOutsideHandler(self, false, 'main', false)

    local csbNode = cc.CSLoader:createNode('promotion/PlazaRromotionLayer.csb'):addTo(self):move(0, -40)
...
--按键监听
function PlazaRromotionLayer:onButtonClickedEvent(tag, sender)
    if tag == WIDGET_TAG.BT_CLOSE then
        self:dismiss()
    end
end

function PlazaRromotionLayer:dismiss(cb)
    self:runAction(
        cc.Sequence:create(
            cc.FadeOut:create(0),
            cc.CallFunc:create(
...
function PlazaRromotionLayer:show(cb)
