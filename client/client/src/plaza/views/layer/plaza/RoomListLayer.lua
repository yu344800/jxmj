...
function RoomListLayer:ctor(scene, isQuickStart)
	...
	if #self.m_tabRoomListInfo == 0 then 
		local text = ccui.Text:create('即将开放，敬请期待', 'fonts/round_body.ttf', 32)
				 :move(display.center)
				 :addTo(self)
	end
end
...
function RoomListLayer:tableCellAtIndex(view, idx)
	...
	if 8 == wLv then
		...
		-- local path = "game/" .. modulestr .. "res/roomlist/title_icon_" .. wLv .. ".png"
		local path = "game/" .. modulestr .. "res/roomlist/roomlist_tag_1.png"
		-- if nil ~= frame then
		-- 	local sp = cc.Sprite:createWithSpriteFrame(frame)
		...
		-- elseif cc.FileUtils:getInstance():isFileExist(path) then		
			display.newSprite(path)
				:setAnchorPoint(0, 0)
				:setPosition(cc.p(self.m_fThree * 0.25, 0))
				:addTo(cell)
		-- else
