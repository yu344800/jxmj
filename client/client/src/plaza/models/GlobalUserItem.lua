...
function GlobalUserItem.reSetData()
	...
	-- GlobalUserItem.szThirdPartyUrl							= "http://wx.qlogo.cn/mmopen/g3MonUZtNHkdmzicIlibx6iaFqAc56vxLSUfpb6n5WKSYVY0ChQKkiaJSgQ1dZuTOgvLLrhJbERQQ4eMsv84eavHiaiceqxibJxCfHe/0"		--第三方头像url
	...
	-- 游戏音效语音
	GlobalUserItem.GameSoundLanguage						= ""
	--游戏背景		
	GlobalUserItem.GameBg									= ""
	--代开
	-- GlobalUserItem.GameSelect								= ""
...
function GlobalUserItem.LoadData()
	...
	GlobalUserItem.GameSoundLanguage = cc.UserDefault:getInstance():getStringForKey("gameSoundLanguage", "pt")
	GlobalUserItem.GameBg = cc.UserDefault:getInstance():getStringForKey('gamebg', "bg1")
...
function GlobalUserItem.setSoundAble(able)
	if GlobalUserItem.bSoundAble == able then
		return
	end
...
function GlobalUserItem.setVoiceAble(able)
	if GlobalUserItem.bVoiceAble == able then
		return
	end
...
-- 游戏方言
function GlobalUserItem.setGameSoundLanguage(name)
	
	GlobalUserItem.GameSoundLanguage = name
	cc.UserDefault:getInstance():setStringForKey("gameSoundLanguage",GlobalUserItem.GameSoundLanguage)
end

--游戏背景
function GlobalUserItem.SetGameBg(name)
	GlobalUserItem.GameBg = name
	cc.UserDefault:getInstance():setStringForKey('gamebg', GlobalUserItem.GameBg)
end
--查询房间数目(普通房間)
function GlobalUserItem.GetGameRoomCount(nKindID)
	...
	for i = 1,#GlobalUserItem.roomlist do
		local serverItem = GlobalUserItem.roomlist[i]

		if tonumber(serverItem[1]) == checkKind then
			if not serverItem[2] then
				break
			end
			local nCount = 0
			local serverList = serverItem[2]
...
function GlobalUserItem.setHasLogon()
	cc.UserDefault:getInstance():setBoolForKey("hasLogon",true)
end
