	...
	if  self._callBack ~= nil then
		...
		elseif type(pData) == "string" then
			...
			elseif errorcode == 3 then
				self._callBack(-1,"与服务器的连接意外关闭了...")	
			-- 无法连接
			elseif errorcode == 7 then
				self._callBack(-1,"无法与服务器建立连接...")
			-- 心跳包发送失败	
			elseif errorcode == 8 then
				self._callBack(-1,"与服务器数据交互异常...")
			-- 发送数据异常
			elseif errorcode == 9 then
				self._callBack(-1,"发送数据效验失败...")
			-- socket创建失败
			elseif errorcode == 10 then
				self._callBack(-1,"与服务器连接创建失败...")
			-- socket hander初始化异常
			elseif errorcode == 11 then
				self._callBack(-1,"网络连接内部消息分发异常...")
			-- socket 超时
			elseif errorcode == 12 then
				self._callBack(-1,"网络连接超时...")
			else
