...
function TaskFrame:onConnectCompeleted()
	...
	elseif self._oprateCode == 3 then	--奖励
		self:sendTaskReward()
	elseif self._oprateCode == 4 then	--分享
		self:sendCheckShare()
	elseif self._oprateCode == 5 then	--获取分享奖励
		self:sendGetShareAward()
	else
		self:onCloseSocket()
...
function TaskFrame:onSocketEvent(main,sub,pData)
	...
	if main == yl.MDM_GP_USER_SERVICE then 		--用户服务
		...
		elseif sub == logincmd.SUB_GP_QUERY_SHARE_AWARD_COUNT_RESULT then 	--分享次数
			self:onSubShareCount(pData)
		else
...
-- 发送查询
function TaskFrame:sendCheckShare()
	local buffer = CCmd_Data:create(4)
	buffer:setcmdinfo(logincmd.MDM_GP_USER_SERVICE, logincmd.SUB_GP_QUERY_SHARE_AWARD_COUNT)
	buffer:pushdword(GlobalUserItem.dwUserID)
	if not self:sendSocketData(buffer) then
		self._callBack(-1,"发送查询分享次数失败！")
	end
end

-- 分享回包
function TaskFrame:onSubShareCount(pData)
	local cmddata = ExternalFun.read_netdata(logincmd.CMD_GP_ShareAwardCount, pData)
	if self._oprateCode == 4 then
		self._callBack(logincmd.SUB_GP_QUERY_SHARE_AWARD_COUNT, cmddata)
	elseif self._oprateCode == 5 then
		self._callBack(logincmd.SUB_GP_QUERY_SHARE_AWARD_COUNT_RESULT, cmddata)
	end
end

-- 获取分享奖励
function TaskFrame:onGetShareAward()
	--操作记录
	self._oprateCode = 5
	if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then
		self._callBack(-1,"建立连接失败！")
...
-- 发送获取奖励
function TaskFrame:sendGetShareAward()
	local buffer = CCmd_Data:create(4)
	buffer:setcmdinfo(logincmd.MDM_GP_USER_SERVICE, logincmd.SUB_GP_GET_SHARE_AWARD)
	buffer:pushdword(GlobalUserItem.dwUserID)
	if not self:sendSocketData(buffer) then
		self._callBack(-1,"发送获取分享钻石失败！")
	end
end
