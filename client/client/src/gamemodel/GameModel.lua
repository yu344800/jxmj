...
function GameModel:onKeyBack()
    if PriRoom:getInstance():isCurrentGameOpenPri(self._gameKind) == true then
...
function GameModel:onCancelResult(cmd_table)
    dump('->onCancelResult 解散消息未处理')
    return false
end

function GameModel:onPriGameEnd(...)
    dump('->onPriGameEnd 结束消息未处理')
end
