...
local PriGameLayer = appdf.req(appdf.GAME_SRC .. 'yule.jysparrow.src.privateroom.PriGameLayer')
-- 登陆服务器CMD 
...
PriFrame.OP_SCOREEXCHANGROOMCARD = cmd_pri_login.SUB_MB_SCORE_EXCHANGE_TO_ROOM_CARD --兑换房卡
...
function PriFrame:onExchangeRoomCard( lCount )
    --操作记录
    self._oprateCode = PriFrame.OP_SCOREEXCHANGROOMCARD
    self._lExchangeScore = lCount
    if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then
        self._callBack(LOGINSERVER(-1),"建立连接失败！")
    end
end
...
function PriFrame:onConnectCompeleted()
    ...
    elseif self._oprateCode == PriFrame.OP_SCOREEXCHANGROOMCARD then-- 发送游戏币兑换房卡
        self:sendExchangeRoomCard()
    else
        self:onCloseSocket()
...
-- 发送游戏币兑换房卡
function PriFrame:sendExchangeRoomCard()
    local buffer = ExternalFun.create_netdata(cmd_pri_login.CMD_GP_ExchangRoomCardByScore)
    buffer:setcmdinfo(cmd_pri_login.MDM_MB_PERSONAL_SERVICE,cmd_pri_login.SUB_MB_SCORE_EXCHANGE_TO_ROOM_CARD)
    buffer:pushdword(GlobalUserItem.dwUserID)
    buffer:pushscore(self._lExchangeScore)
    buffer:pushstring(GlobalUserItem.szMachine,yl.LEN_MACHINE_ID)
    if not self:sendSocketData(buffer) and nil ~= self._callBack then
        self._callBack(LOGINSERVER(-1),"发送兑换房卡失败！")
    end
end
...
function PriFrame:sendEnterPrivateGame()
    if nil ~= self._gameFrame and self._gameFrame:isSocketServer() then
        ...
        dump(PriRoom:getInstance().isCanJoinGame)
        if PriRoom:getInstance().isCanJoinGame then--进入
...
function PriFrame:onSocketEvent( main,sub,pData )
    ...
    if cmd_pri_login.MDM_MB_PERSONAL_SERVICE == main then
        ...
        elseif cmd_pri_login.SUB_MB_PERSONAL_PARAMETER == sub then                  -- 私人房间属性
            ...
            -- dump(PriRoom:getInstance().m_tabRoomOption,"私人房属性")
        ...
        elseif cmd_pri_login.SUB_GP_EXCHANGE_ROOM_CARD_RESULT == sub then           -- 兑换结果
...
function PriFrame:onSubSearchRoomResult( pData )
    ...
    if 0 == cmd_table.dwServerID then
        if nil ~= self._callBack then
            self._callBack(-1, "该房间ID不存在, 请重新输入!")
    ...
    --是加入还是观战
    PriRoom:getInstance().isCanJoinGame  = cmd_table.cbCanJoinGame
    -- 动作定义
...
function PriFrame:onSubFeeParameter( pData )
    print('PriFrame:onSubFeeParameter')
    ...
    for idx = 1, count do
        ...
        -- 过滤 局数 和 费用 都一致的数据
        local exist = false
        local list = PriRoom:getInstance().m_tabFeeConfigList

        for i=1, #list do
            if list[i].dwDrawCountLimit == param.dwDrawCountLimit and list[i].lFeeScore == param.lFeeScore then
                exist = true
            end
        end
...
function PriFrame:onSubGameServerResult( pData )
    ...
    PriRoom:getInstance().isCanJoinGame = true
    -- 发送登陆
    PriRoom:getInstance():onLoginRoom(cmd_table.dwServerID)
    return false
end
...
function PriFrame:onSubCancelRequest( pData )
    local cmd_table = ExternalFun.read_netdata(cmd_pri_game.CMD_GR_CancelRequestStatus, pData)
    -- 自己不处理
...
function PriFrame:onSubTableTip( pData )
    ...
    PriRoom:getInstance().m_tabPriRoomInfo = {}
    PriRoom:getInstance().m_tabPriRoomInfo.dwUserID = cmd_table.dwTableOwnerUserID
    PriRoom:getInstance().m_tabPriRoomInfo.cbGameRule = cmd_table.cbGameRule
    PriRoom:getInstance().m_tabPriRoomInfo.szUserNicName = cmd_table.szUserNicName
    PriRoom:getInstance().m_tabPriRoomInfo.bRoomGameStart = cmd_table.bRoomGameStart
...
function PriFrame:onSubTableEnd( pData )
    ...
    if nil ~= self._callBack then
        self._callBack(GAMESERVER(cmd_pri_game.SUB_GR_PERSONAL_TABLE_END), cmd_table)
        -- dump(cmd_table,"cmd_pri_game.SUB_GR_PERSONAL_TABLE_END")
    end
