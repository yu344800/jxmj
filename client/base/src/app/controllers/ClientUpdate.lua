...
function ClientUpdate:UpdateFile()
	...
	downFileAsync(url,fileinfo[2],dstpath,function(main,sub)
			...
			if main == appdf.DOWN_PRO_INFO then --进度信息
				if nil ~= this._listener and type(this._listener.updateProgress) == "function" then
			...
			elseif main == appdf.DOWN_COMPELETED then --下载完毕
				...
				if sub == 28 and self._retryCount > 0 then
					...
					if nil ~= this._listener and type(this._listener.updateResult) == "function" then
						this._listener:updateResult(false,"下载失败,main:" .. main .. " ## sub:" .. sub .. "## file:" .. fileinfo[2]) --失败信息
					end
				end
			end
		end)
