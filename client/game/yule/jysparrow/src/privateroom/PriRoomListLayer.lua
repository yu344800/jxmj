...
function PriRoomListLayer:ctor( scene )
    ...
    self._scene = scene
    -- 加载csb资源
    local rootLayer, csbNode = ExternalFun.loadRootCSB("room/PriRoomsLayer.csb", self)
    ...
    -- 加入房间
    local btn = appdf.getNodeByName(csbNode, 'Button_join')
    btn:setTag(BTN_JOIN_PRIROOM)
    btn:addTouchEventListener(touchFunC)
    local joinBtn = btn

    -- 创建房间
    btn = appdf.getNodeByName(csbNode, 'Button_create')
